...
A shared folder was set up on a central server, accessible to all retailers
Each retailer/regional manager uploads their own sales spreadsheet to this folder at the end of each reporting period
Power BI connects directly to the folder (not a single file) and automatically combines every spreadsheet inside it into one unified table — PureSip Data Folder — using Power Query
Because the connection targets the folder itself, the model refreshes automatically whenever a new or updated spreadsheet is dropped in, with zero manual re-consolidation
...
Visuals included
KPI Tracker | Contact — a gauge-style card tracking one regional contact's sales against a fixed $250,000 target, defaulting to contact Stuart (Target), currently 4.46% below target
Sales | Month Analysis (bar/trend chart) — monthly sales across 2022, with August highlighted as the peak month
Sales | Regional Analysis (map) — sales by U.S. region/state
Sales | Retailer Analysis (bar chart) — sales by retail chain
Sales | Brand Analysis (bar chart) — sales by beverage brand
Beverage Brand / Contact slicers — interactive filtering across the whole report
...
💡 Recommendations
Investigate Target's underperformance against its $250K benchmark — a 4.46% shortfall flagged directly by the KPI tracker, worth a closer look at what's driving it (pricing, assortment, promotional support).
Double down on West Coast / California relationships, given they account for nearly half of total sales — consider whether this concentration is an opportunity to expand further or a risk worth diversifying against.
Review brand-level pricing strategy — Dasani Water's higher price point delivering the top revenue result suggests there may be room to test pricing on other high-volume, lower-margin brands like Coca-Cola.
Formalize payment-term enforcement with retailers exceeding the standard 30-day payment window to protect cash flow.
Extend the folder-connection model to additional data sources (e.g. marketing spend, inventory) now that the automated refresh pipeline is proven, to build a more complete view of the business without added manual work.
